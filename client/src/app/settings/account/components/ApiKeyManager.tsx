...
export function ApiKeyManager() {
  ...
  const handleDeleteApiKey = async (keyId: string, keyName: string | null) => {
    if (!confirm(t('Are you sure you want to delete the API key "{name}"?', { name: keyName || t("Unnamed") }))) {
      return;
    }
  ...
  const copyToClipboard = (text: string) => {
    ...
    toast.success(tc("Copied to clipboard"));
  };
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-950 p-3 border border-yellow-200 dark:border-yellow-800">
              <p className="text-xs text-yellow-800 dark:text-yellow-200">
                {t("Store this key securely. It won't be displayed again.")}
              </p>
            </div>
