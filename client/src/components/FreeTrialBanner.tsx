    ...
    return (
      <div className="mt-4 px-4 py-3 rounded-lg border border-blue-100 dark:border-blue-400/30 bg-blue-50/80 dark:bg-blue-900/20 text-sm flex justify-between items-center">
        <div className="text-blue-700 dark:text-blue-300 flex items-center font-medium">
          ...
          <span>
            {t("Free trial: {days} days remaining – Using {used} of {limit} events", { days: daysRemaining, used: formatNumber(eventsUsed), limit: formatNumber(eventLimit) })}
          </span>
        </div>
        <Button
          variant="outline"
        ...
        >
          <Link href="/subscribe">
            {tc("Upgrade")} <ArrowRight className="ml-1 h-3 w-3" />
          </Link>
        </Button>
      </div>
    );
